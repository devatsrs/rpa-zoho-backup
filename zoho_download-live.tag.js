...
var automation_start_time = Date.now(); casper.echo('' + Date().toLocaleString());
...
else if (tagui_language.toLowerCase() == 'english') casper.test.comment(echo_string);
else {var translated_string = translate(echo_string,'to',tagui_language.toLowerCase()); casper.test.comment(translated_string);
...
default: return "test.fail('ERROR - cannot understand step " + live_line.replace(/'/g,'\\\'') + "')";}}
...
if (!fs.exists('tagui.sikuli/tagui_sikuli.in')) return "test.fail('ERROR - cannot initialise tagui_sikuli.in')";
if (!fs.exists('tagui.sikuli/tagui_sikuli.out')) return "test.fail('ERROR - cannot initialise tagui_sikuli.out')";
...
use_ocr + ") " + "test.fail('ERROR - cannot find image file "+input_params+"'); " +
"else test.fail('ERROR - cannot find "+input_params+" on screen'); " + other_actions;}
...
if (!fs.exists('tagui_r/tagui_r.in')) return "test.fail('ERROR - cannot initialise tagui_r.in')";
if (!fs.exists('tagui_r/tagui_r.out')) return "test.fail('ERROR - cannot initialise tagui_r.out')";
return "r_result = ''; if (!r_step('"+input_intent+"')) test.fail('ERROR - cannot execute R command(s)'); else {r_result = fetch_r_text(); clear_r_text(); try {r_json = JSON.parse(r_result);} catch(e) {r_json = JSON.parse('null');}}";}
...
if (!fs.exists('tagui_py/tagui_py.in')) return "test.fail('ERROR - cannot initialise tagui_py.in')";
if (!fs.exists('tagui_py/tagui_py.out')) return "test.fail('ERROR - cannot initialise tagui_py.out')";
return "py_result = ''; if (!py_step('"+input_intent+"')) test.fail('ERROR - cannot execute Python command(s)'); else {py_result = fetch_py_text(); clear_py_text(); try {py_json = JSON.parse(py_result);} catch(e) {py_json = JSON.parse('null');}}";}
...
if (chrome_id == 0) return "test.fail('ERROR - step only supported in live mode using Chrome browser')";
...
if (params == '') return "test.fail('ERROR - target missing for " + raw_intent + "')";
else if (check_tx(params)) return "chrome.click(tx('" + params + "'))";
else return "test.fail('ERROR - cannot find " + params + "')";}
...
if ((param1 == '') || (param2 == '')) return "test.fail('ERROR - target/text missing for " + raw_intent + "')";
...
else return "test.fail('ERROR - cannot find " + param1 + "')";}
...
return call_sikuli(abs_intent,abs_param1);} // use sikuli visual automation as needed
if ((param1 == '') || (param2 == '')) return "test.fail('ERROR - target/option missing for " + raw_intent + "')";
...
if ((param1 == '') || (param2 == '')) return "test.fail('ERROR - target/variable missing for " + raw_intent + "')";
...
return call_sikuli(abs_intent,abs_params,'test.comment(fetch_sikuli_text()); clear_sikuli_text();');}
if (params.toLowerCase() == 'page') return "test.comment('" + raw_intent + "' + ' - ' + '\\n' + chrome.getHTML())";
if (params == '') return "test.fail('ERROR - target missing for " + raw_intent + "')";
else if (check_tx(params)) return "test.comment(chrome.fetchText(tx('" + params + "')).trim())";else return "test.fail('ERROR - cannot find " + params + "')";}

function upload_intent(raw_intent) {raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
var param1 = (params.substr(0,params.indexOf(' as '))).trim();
var param2 = (params.substr(4+params.indexOf(' as '))).trim();
if ((param1 == '') || (param2 == '')) return "test.fail('ERROR - filename missing for " + raw_intent + "')";
...
if (param2 == '') return "test.fail('ERROR - location missing for " + raw_intent + "')";
...
return "test.fail('ERROR - step not supported in live mode, it requires creating CasperJS event')";}

function echo_intent(raw_intent) {raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.comment('')"; else return "test.comment('" + add_concat(params) + "')";}
...
if (params == '') return "test.fail('ERROR - variable missing for " + raw_intent + "')";
...
var param1 = (params.substr(0,params.lastIndexOf(' to '))).trim();
var param2 = (params.substr(4+params.lastIndexOf(' to '))).trim();
if (params == '') return "test.fail('ERROR - filename missing for " + raw_intent + "')";
else if (params.lastIndexOf(' to ') > -1)
return "var fs = require('fs'); " + param2 + " = ''; if (fs.exists('" + abs_file(param1) + "')) " + param2 +  " = fs.read('" + abs_file(param1) + "').trim(); else test.fail('ERROR - cannot find file " + param1 + "')"; else
return "test.fail('ERROR - variable missing for " + raw_intent + "')";}
...
return "test.fail('ERROR - waiting for some time is not relevant in live mode')";}

function live_intent(raw_intent) {raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
return "test.fail('ERROR - you are already in live mode, type done to quit live mode')";}

function ask_intent(raw_intent) {raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
return "test.fail('ERROR - step is not relevant in live mode, set ask_result directly')";}

function keyboard_intent(raw_intent) {raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - keys to type missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - down / up missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - API URL missing for " + raw_intent + "')";
...
return "test.fail('ERROR - step not supported in live mode, as run output cannot be retrieved')";}
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - statement missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - R command(s) missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - Python command(s) missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - Sikuli command(s) missing for " + raw_intent + "')";
...
var params = ((raw_intent + ' ').substr(1+(raw_intent + ' ').indexOf(' '))).trim();
if (params == '') return "test.fail('ERROR - time in seconds missing for " + raw_intent + "')";
...
var flow_path = 'D:/laragon/www/rpa-zoho-backup';

casper.test.begin('D:/laragon/www/rpa-zoho-backup/zoho_download-live.tag', 0, function(test) {
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://accounts.zoho.in/signin'}); sleep(1000);
techo('https://accounts.zoho.in/signin' + ' - ' + chrome.getTitle());});

casper.then(function() {techo('wait 10 seconds');});
casper.then(function() {casper.wait(10000, function() {});});

// check if login screen
casper.then(function() {if ((exist('email')))
{ // start of code block

casper.then(function() {{techo('type email as holly@lgpharma.com.au');
casper.waitFor(function check() {return check_tx('email');},
function then() {chrome.sendKeys(tx('email'),'holly@lgpharma.com.au');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find email').exit(); this.wait(0);});}});

} // end of code block
});

casper.then(function() {if ((exist('nextbtn')))
{ // start of code block

casper.then(function() {{techo('click nextbtn');
casper.waitFor(function check() {return check_tx('nextbtn');},
function then() {chrome.click(tx('nextbtn'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find nextbtn').exit(); this.wait(0);});}});

} // end of code block
});

// type password as 12345678
casper.then(function() {if ((exist('password')))
{ // start of code block

casper.then(function() {{techo('type password as H0ll1fer');
casper.waitFor(function check() {return check_tx('password');},
function then() {chrome.sendKeys(tx('password'),'H0ll1fer');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find password').exit(); this.wait(0);});}});
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://crm.zoho.com/crm/org721637160/settings/data-backup'}); sleep(1000);
techo('https://crm.zoho.com/crm/org721637160/settings/data-backup' + ' - ' + chrome.getTitle());});
...
casper.then(function() {techo('wait 30 seconds');});
casper.then(function() {casper.wait(30000, function() {});});

// download to [D:\\tagui]
// Download Data File
casper.then(function() {if ((exist('dbDataLink')))
{ // start of code block

casper.then(function() {{techo('click //*[@id="dbDataLink"]/div/a');
casper.waitFor(function check() {return check_tx('//*[@id="dbDataLink"]/div/a');},
function then() {chrome.click(tx('//*[@id="dbDataLink"]/div/a'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find //*[@id="dbDataLink"]/div/a').exit(); this.wait(0);});}});
...
casper.then(function() {test.comment('Download link not present');});

} // end of code block
});

casper.then(function() {if ((!exist('dbAttachmentLink')))
{ // start of code block

casper.then(function() {test.comment('Download attachment link not present');});
...
casper.then(function() {{techo('click //*[@id="dbAttachmentLink"]/div/a');
casper.waitFor(function check() {return check_tx('//*[@id="dbAttachmentLink"]/div/a');},
function then() {chrome.click(tx('//*[@id="dbAttachmentLink"]/div/a'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find //*[@id="dbAttachmentLink"]/div/a').exit(); this.wait(0);});}});
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://outlook.office.com/mail/'}); sleep(1000);
techo('https://outlook.office.com/mail/' + ' - ' + chrome.getTitle());});

// wait 1 seconds
casper.then(function() {if ((exist('idSIButton9')))
{ // start of code block

casper.then(function() {{techo('type email as orders@littlegreenpharma.com');
casper.waitFor(function check() {return check_tx('email');},
function then() {chrome.sendKeys(tx('email'),'orders@littlegreenpharma.com');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find email').exit(); this.wait(0);});}});

casper.then(function() {{techo('click idSIButton9');
casper.waitFor(function check() {return check_tx('idSIButton9');},
function then() {chrome.click(tx('idSIButton9'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find idSIButton9').exit(); this.wait(0);});}});

casper.then(function() {techo('wait 1 seconds');});
casper.then(function() {casper.wait(1000, function() {});});

casper.then(function() {{techo('type password as Suk47276');
casper.waitFor(function check() {return check_tx('password');},
function then() {chrome.sendKeys(tx('password'),'Suk47276');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find password').exit(); this.wait(0);});}});
...
// click new message
casper.then(function() {{techo('click D:/laragon/www/rpa-zoho-backup/new_message.png'); var fs = require('fs');
if (!sikuli_step('click D:/laragon/www/rpa-zoho-backup/new_message.png')) if (!fs.exists('D:/laragon/www/rpa-zoho-backup/new_message.png') && !false)
{test.fail('ERROR - cannot find image file D:/laragon/www/rpa-zoho-backup/new_message.png').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find D:/laragon/www/rpa-zoho-backup/new_message.png on screen').exit(); this.wait(0);}}});
...
casper.then(function() {{techo('type D:/laragon/www/rpa-zoho-backup/email_to.png as sdeven@gkblabs.com'); var fs = require('fs');
if (!sikuli_step('type D:/laragon/www/rpa-zoho-backup/email_to.png as sdeven@gkblabs.com')) if (!fs.exists('D:/laragon/www/rpa-zoho-backup/email_to.png') && !false)
{test.fail('ERROR - cannot find image file D:/laragon/www/rpa-zoho-backup/email_to.png').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find D:/laragon/www/rpa-zoho-backup/email_to.png on screen').exit(); this.wait(0);}}});

casper.then(function() {{techo('keyboard [tab]'); var fs = require('fs');
if (!sikuli_step('keyboard [tab]')) if (!fs.exists('[tab]') && !false)
{test.fail('ERROR - cannot find image file [tab]').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find [tab] on screen').exit(); this.wait(0);}}});

// switch to subject
//              click email_subject.png
casper.then(function() {{techo('click Add a subject');
casper.waitFor(function check() {return check_tx('Add a subject');},
function then() {chrome.click(tx('Add a subject'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find Add a subject').exit(); this.wait(0);});}});

// type subject
casper.then(function() {{techo('type Add a subject as Backup Automation Completed');
casper.waitFor(function check() {return check_tx('Add a subject');},
function then() {chrome.sendKeys(tx('Add a subject'),'Backup Automation Completed');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find Add a subject').exit(); this.wait(0);});}});

// switch to message body
// click email_body.png
casper.then(function() {{techo('click //br');
casper.waitFor(function check() {return check_tx('//br');},
function then() {chrome.click(tx('//br'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find //br').exit(); this.wait(0);});}});

// type body
casper.then(function() {{techo('type //br as  Backup Automation Completed');
casper.waitFor(function check() {return check_tx('//br');},
function then() {chrome.sendKeys(tx('//br'),'Backup Automation Completed');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find //br').exit(); this.wait(0);});}});

// click send button
casper.then(function() {{techo('click D:/laragon/www/rpa-zoho-backup/email_send.png'); var fs = require('fs');
if (!sikuli_step('click D:/laragon/www/rpa-zoho-backup/email_send.png')) if (!fs.exists('D:/laragon/www/rpa-zoho-backup/email_send.png') && !false)
{test.fail('ERROR - cannot find image file D:/laragon/www/rpa-zoho-backup/email_send.png').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/laragon/www/rpa-zoho-backup/zoho_download-live_error.png');
test.fail('ERROR - cannot find D:/laragon/www/rpa-zoho-backup/email_send.png on screen').exit(); this.wait(0);}}});

casper.then(function() {techo('\n' + chrome.getCurrentUrl() + ' - ' + chrome.getTitle());
dummy_echo('FINISH - test finished - ' + ((Date.now()-automation_start_time)/1000).toFixed(1) + 's\n');});
