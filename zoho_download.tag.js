...
casper.test.begin('D:\\laragon\\www\\rpa-zoho-backup\\zoho_download.tag', 0, function(test) {
...
casper.then(function() {{techo('type email as devensitapara@gmail.com');
casper.waitFor(function check() {return check_tx('email');},
function then() {chrome.sendKeys(tx('email'),'devensitapara@gmail.com');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download_error.png');
...
casper.then(function() {{techo('type password as 12345678');
casper.waitFor(function check() {return check_tx('password');},
function then() {chrome.sendKeys(tx('password'),'12345678');},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download_error.png');
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://crm.zoho.in/crm/ShowHomePage.do'}); sleep(1000);
techo('https://crm.zoho.in/crm/ShowHomePage.do' + ' - ' + chrome.getTitle());});

// wait 1 seconds
// Go to settings  Gear icon
casper.then(function() {{techo('click tab_Setup');
casper.waitFor(function check() {return check_tx('tab_Setup');},
function then() {chrome.click(tx('tab_Setup'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download_error.png');
test.fail('ERROR - cannot find tab_Setup').exit(); this.wait(0);});}});

// wait 1 seconds
// Click export under data administator
casper.then(function() {{techo('click #data_export > link-to > a');
casper.waitFor(function check() {return check_tx('#data_export > link-to > a');},
function then() {chrome.click(tx('#data_export > link-to > a'));},
function timeout() {chrome.capture('D:/laragon/www/rpa-zoho-backup/zoho_download_error.png');
test.fail('ERROR - cannot find #data_export > link-to > a').exit(); this.wait(0);});}});
